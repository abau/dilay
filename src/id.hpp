...
class Id {
  public: Id (const Id& id) : _id (id.primitive ()) {}
          Id ()             : _id (0) {}

    friend class IdObject;

    bool operator== (const Id& id) const { return this->_id == id.primitive (); }
    bool operator!= (const Id& id) const { return this->_id != id.primitive (); }

    IdPrimitive primitive () const { return this->_id; }
...
namespace std {
  template <> struct hash <Id> {
    std::size_t operator () (const Id& id) const {
      return std::hash <IdPrimitive> () (id.primitive ());
    }
  };
}
...
class IdObject {
  ...
  private:
    ...
    static IdPrimitive nextIdPrimitive ();
};
