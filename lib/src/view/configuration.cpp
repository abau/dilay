...
namespace
{
  ...
  void addVectorEdit (ViewGlWidget& glWidget, ViewTwoColumnGrid& grid, const std::string& path,
                      const QString& label)
  {
    ...
    QObject::connect (&edit, &ViewVectorEdit::vectorEdited,
                      [&glWidget, path](const glm::vec3& v) { setAndUpdate (glWidget, path, v); });
    grid.add (label, edit);
  }

  void addFloatEdit (ViewGlWidget& glWidget, ViewTwoColumnGrid& grid, const std::string& path,
                     const QString& label, float min, float max)
  {
    QLineEdit& edit = ViewUtil::lineEdit (min, glWidget.state ().config ().get<float> (path), max);
    ViewUtil::connectFloat (edit, [&glWidget, path](float f) { setAndUpdate (glWidget, path, f); });
    grid.add (label, edit);
  }

  void addIntEdit (ViewGlWidget& glWidget, ViewTwoColumnGrid& grid, const std::string& path,
                   const QString& label, int min, int max)
  {
    QLineEdit& edit = ViewUtil::lineEdit (min, glWidget.state ().config ().get<int> (path), max);
    ViewUtil::connectInt (edit, [&glWidget, path](int i) { setAndUpdate (glWidget, path, i); });
    grid.add (label, edit);
  }
  ...
  QWidget* makeCameraPage (ViewGlWidget& glWidget)
  {
    ...
    addFloatEdit (glWidget, *grid, "editor/camera/near-clipping",
                  QObject::tr ("Near clipping plane"), Util::epsilon (), 1.0f);
  ...
  QWidget* makeMiscPage (ViewGlWidget& glWidget)
  {
    ...
    addIntEdit (glWidget, *grid, "editor/undo-depth", QObject::tr ("Undo depth"), 1,
                Util::maxInt ());
    addIntEdit (glWidget, *grid, "window/initial-width", QObject::tr ("Initial window width"), 1,
                Util::maxInt ());
    addIntEdit (glWidget, *grid, "window/initial-height", QObject::tr ("Initial window height"), 1,
                Util::maxInt ());

    addVectorEdit (glWidget, *grid, "editor/axis/scaling", QObject::tr ("Axis scaling"));
    addVectorEdit (glWidget, *grid, "editor/axis/arrow-scaling",
                   QObject::tr ("Axis-arrow scaling"));
