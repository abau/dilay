...
namespace {
  ...
  float weight (const glm::vec3& p1, const glm::vec3& p2, const glm::vec3& p3) {
    ...
    const float weight    = glm::abs (bestAngle - a1)
                          + glm::abs (bestAngle - a2)
                          + glm::abs (bestAngle - a3);
  ...
  std::vector <Triangle> triangelize (const std::vector <Vertex>& vs) {
    ...
    std::function <void (size_t, size_t)> trace = [&vs, &triangles, &minIndices, &trace]
                                                  (size_t i, size_t k)
    {
      if (i + 2 == k) {
    ...
    for (size_t i = 0; i <= n - 3; i++) {
      weights.at (i, i + 2) = weight ( vs.at (i).position
                                     , vs.at (i + 1).position
                                     , vs.at (i + 2).position );
    }
    ...
    do {
      ...
      for (size_t i = 0; i <= n - j - 1; i++) {
        ...
        for (size_t m = i + 1; m < k; m++) {
          const float w = weights.at (i,m) 
                        + weights.at (m,k) 
                        + weight ( vs.at (i).position
                                 , vs.at (m).position
                                 , vs.at (k).position );
...
namespace ToolTrimMeshAction {
  ...
  bool trimMesh (WingedMesh& mesh, ToolTrimMeshBorder& border) {
    std::vector <unsigned int> newIndices;

    Mesh newMesh = mesh.makePrunedMesh (nullptr, &newIndices);
    if (newIndices.empty () == false) {
      border.setNewIndices (newIndices); 
    }
    newMesh = trimMesh (newMesh, border);
